...
def get_inst_survey() -> list:
    ...
    try:
        ...
        if df is not None and len(df) > 0:
            if "fund_nums" not in df.columns:
                df["fund_nums"] = 0
            df["fund_nums"] = pd.to_numeric(df["fund_nums"], errors="coerce")
            df["fund_nums"] = df["fund_nums"].fillna(0)
    ...
    if forecast:
        ...
        for item in forecast[:5]:
            fc_rows += (
                ...
                "<td style='padding:5px 8px;text-align:right;color:#d63031;font-weight:500'>+"
                + str(item.get("pct_max","")) + "%</td>"
                "<td style='padding:5px 8px;color:#888;font-size:11px'>"
                + str(item.get("ann_date","")) + "</td></tr>"
            )
        extra_sections += (
            ...
            "</tr></thead><tbody>" + fc_rows + "</tbody></table></div>"
        )
    if broker_rec:
        br_rows = ""
        for item in broker_rec[:5]:
            br_rows += (
                "<tr><td style='padding:5px 8px'>" + str(item.get("name","")) + "</td>"
                "<td style='padding:5px 8px;text-align:center;color:#6c5ce7;font-weight:500'>"
                + str(item.get("broker_count","")) + "家</td>"
                "<td style='padding:5px 8px;color:#888;font-size:11px'>"
                + str(item.get("brokers","")) + "</td></tr>"
            )
        extra_sections += (
            "<div style='margin-bottom:16px'>"
            ...
            "</tr></thead><tbody>" + br_rows + "</tbody></table></div>"
        )
