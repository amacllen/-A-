...
def get_daily_data() -> tuple:
    ...
    try:
        ...
        price_ok = price is not None and not price.empty
        basic_ok = basic is not None and not basic.empty
...
【重要规则】
1. 只分析今天（{TODAY_CN}）的数据
2. 昨日数据仅作背景参考，不代表今日走势
3. 新闻和公告为空时直接说明，不推断不编造
4. 标的推荐必须有数据支撑，无数据不推荐具体标的
...
【昨日（{ydate_str}）资金背景（仅供参考）】
北向资金昨日：{north}亿，重仓：{north_top}
昨日龙虎榜机构席位：{dt_names}
昨日主力净流入前五：{mf_text}

注意：涨停板、连板天梯、游资今日动向、THS热榜均为收盘后入库，上午无法获取，将在今日18:00收盘报告中提供。

请输出上午快报：

**一、今日市场背景判断**
基于今日新闻和昨日资金数据，判断今日市场可能的运行方向
（如新闻为空，直接说明今日暂无明显政策催化）
...
**一、今日市场驱动逻辑判断**
当前市场最看重什么维度？这个判断如何影响选股方向？

**二、核心数据解读**
基于真实数据解读资金、北向、板块信号（数据缺失直接标注）

**三、明日重点关注标的（2-3只）**
必须同时满足技术面+资金面+政策/基本面至少两个维度
给出：入选理由、关键价位、操作建议、综合评分（1-10）

**四、明日操作策略**
首选标的、止损原则、需要回避的方向

直接给结论，数据缺失的维度直接说明，不编造。"""
