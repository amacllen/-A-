...
def fetch_policy_news() -> list:
    ...
    try:
        ...
        if df_policy is not None and len(df_policy) > 0:
            policy_keywords_gov = ["政策","规划","意见","办法","通知","决定","方案","标准","条例"]
            for _, row in df_policy.head(50).iterrows():
                title = str(row.get("title", ""))
                if any(k in title for k in POLICY_KEYWORDS) and any(k in title for k in policy_keywords_gov):
                    result.append(f"[公告政策] {title[:150]}")
    except Exception as e:
        print(f"  政策公告失败: {e}")
...
def get_today_news() -> list:
    ...
    result = []
    try:
        # Tushare news接口：start_date和end_date格式为 YYYY-MM-DD HH:MM:SS
        today_start = _bj.strftime("%Y-%m-%d") + " 00:00:00"
        today_end   = _bj.strftime("%Y-%m-%d") + " 23:59:59"
        for src in ["cls", "sina", "10jqka"]:
            try:
                df = pro.news(src=src, start_date=today_start, end_date=today_end,
                              fields="title,content,pub_time")
                if df is None or len(df) == 0:
                    continue
                # 严格过滤：只保留今天的新闻
                ...
                for _, row in df.head(80).iterrows():
                    ...
                    if today_str not in pub_time:
                        continue
                    text = str(row.get("title", "")) + str(row.get("content", ""))[:80]
                    if any(k in text for k in POLICY_KEYWORDS):
                        result.append({
                            "title":    str(row.get("title", ""))[:150],
                            "pub_time": pub_time[11:16] if len(pub_time) > 10 else pub_time,
                        })
                if result:
                    print(f"  Tushare新闻({src})：{len(result)}条政策相关")
                    break
            except Exception as e:
                print(f"  Tushare新闻({src})失败: {e}")
        if not result:
            print("  今日新闻：暂无（将显示为空，不使用历史数据）")
    except Exception as e:
...
【重要规则——必须严格遵守】
1. 只分析今天（{TODAY_CN}）的数据，禁止引用任何历史事件、过去的政策或旧新闻
2. 如果某项数据显示"暂无"或"0"，直接说明数据未获取，不得基于历史信息推断
3. 股票推荐必须有今日真实数据支撑（涨停/游资/热榜），没有数据不推荐
4. 政策信号必须是今日发布的，不是今日的一律不引用
