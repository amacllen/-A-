...
def fetch_cls_news() -> list:
    """财联社快讯（多函数名容错）"""
    result = []
    for func_name in ["stock_telegraph_cls", "stock_cls_telegraph", "news_cls_telegraph"]:
        try:
            ...
            if result:
                print(f"  财联社({func_name})：{len(result)}条")
                return result[:8]
        except Exception as e:
            print(f"  财联社{func_name}失败: {e}")
    # 备选：东方财富
    try:
        df = ak.stock_news_em(symbol="000001")
        if df is not None and len(df) > 0:
            for _, row in df.head(30).iterrows():
                text = str(row.get("新闻标题", row.get("title", "")))
                if any(k in text for k in POLICY_KEYWORDS):
                    result.append(f"[东财] {text[:180]}")
        print(f"  东方财富备选：{len(result)}条")
    except Exception as e:
        print(f"  东方财富失败: {e}")
    return result[:8]


def fetch_xinhua_rss() -> list:
    """新华社RSS（多个频道）"""
    result = []
    feeds = [
    ...
    for url in feeds:
        try:
            ...
            print(f"  新华社RSS失败({url[-30:]}): {e}")
    return result[:8]


def fetch_rsshub_policy() -> list:
    """
    通过RSSHub抓取多个政府官网政策源
    RSSHub是开源RSS聚合器，提供稳定的政府网站RSS接口
    """
    result = []
    # RSSHub公共实例 + 备用实例
    rsshub_bases = [
            ...
            try:
                ...
                for item in root.iter("item"):
                    ...
                    if any(k in title for k in POLICY_KEYWORDS):
                        ...
                        count += 1
                if count > 0:
                    print(f"  {name}(RSSHub)：{count}条")
                fetched = True
                break
            except Exception as e:
                continue
        if not fetched:
            print(f"  {name} RSSHub所有实例失败，尝试直接爬取...")
            # 降级：直接爬取官网
            gov_direct = {
                "国务院": "https://www.gov.cn/govweb/zhengce/zuixin/",
                "发改委": "https://www.ndrc.gov.cn/xwdt/xwfb/",
                "工信部": "https://www.miit.gov.cn/jgsj/index.html",
            }
            if name in gov_direct:
                try:
                    r = requests.get(gov_direct[name], timeout=8,
                                     headers={"User-Agent": "Mozilla/5.0"})
                    r.encoding = "utf-8"
                    titles = re.findall(r'<a[^>]*href[^>]*>([^<]{8,60})</a>', r.text)
                    for t in titles[:30]:
                        t = t.strip()
                        if any(k in t for k in POLICY_KEYWORDS):
                            result.append(f"[{name}] {t[:120]}")
                except Exception:
                    pass
        time.sleep(0.3)
    return result[:12]
...
def fetch_xueqiu_sentiment(stock_codes: list) -> dict:
    """
    雪球舆情数据：获取指定标的的讨论热度和情绪
    stock_codes: 如 ["SH688017", "SZ002472"]
    """
    sentiment = {}
    headers = {
    ...
    try:
        ...
        for code in stock_codes[:6]:  # 最多查6只，避免太慢
            try:
                # 查询个股讨论数
                url = f"https://xueqiu.com/v4/stock/quote.json?code={code}"
                ...
                if resp.status_code == 200:
                    ...
                    if stock_data:
                        sentiment[code] = {
                            ...
                            "followers":    stock_data.get("followers", 0),
                            "change_pct":   stock_data.get("percent", 0),
                            "volume":       stock_data.get("volume", 0),
                        }
                time.sleep(0.5)
            except Exception as e:
                print(f"  雪球{code}失败: {e}")
    except Exception as e:
        print(f"  雪球session失败: {e}")
    return sentiment
...
def fetch_market_sentiment() -> dict:
    ...
    try:
        ...
        # 情绪判断
        up_ratio = result["up_count"] / max(result["up_count"] + result["down_count"], 1)
        if up_ratio > 0.65:
            result["sentiment"] = "强势偏多"
        elif up_ratio > 0.55:
            result["sentiment"] = "温和偏多"
        elif up_ratio > 0.45:
            result["sentiment"] = "中性震荡"
        elif up_ratio > 0.35:
            result["sentiment"] = "温和偏空"
        else:
            result["sentiment"] = "弱势偏空"

        print(f"  市场情绪：{result['sentiment']}，涨{result['up_count']}跌{result['down_count']}，涨停{result['limit_up']}家")
    except Exception as e:
        print(f"  市场情绪获取失败: {e}")
    return result
...
def fetch_sector_flow() -> list:
    """板块资金流向（今日哪些行业板块在吸引主力资金）"""
    result = []
    try:
        # 尝试行业板块资金流向
        df = ak.stock_sector_fund_flow_rank(symbol="今日", indicator="行业资金流向")
        if df is not None and len(df) > 0:
            df["主力净流入-净额"] = pd.to_numeric(df.get("主力净流入-净额", 0), errors="coerce")
            top = df.nlargest(8, "主力净流入-净额")
            for _, row in top.iterrows():
                net = safe_float(row.get("主力净流入-净额", 0))
                if net > 0:
                    result.append({
                        "sector":      row.get("名称", ""),
            ...
            print(f"  板块资金流向：{len(result)}个板块")
    except Exception as e:
        print(f"  板块资金流向失败: {e}")
        # 备选：概念板块
        try:
            df2 = ak.stock_board_concept_fund_flow_rank(indicator="今日")
            if df2 is not None and len(df2) > 0:
                df2["主力净流入-净额"] = pd.to_numeric(df2.get("主力净流入-净额", 0), errors="coerce")
                top2 = df2.nlargest(8, "主力净流入-净额")
...
def fetch_policy_news() -> list:
    ...
    all_news.extend(fetch_xinhua_rss())
    all_news.extend(fetch_rsshub_policy())

    # 去重
    seen = set()
    deduped = []
    for item in all_news:
        key = item[6:30]  # 去掉来源标签后的前24个字符作为去重key
        if key not in seen:
            seen.add(key)
            deduped.append(item)

    print(f"  政策新闻合计（去重后）：{len(deduped)} 条")
    return deduped[:25]
...
def intraday_capital_flow() -> list:
    ...
    try:
        ...
        df["主力净流入-净额"] = pd.to_numeric(df.get("主力净流入净额", df.get("主力净流入-净额", 0)), errors="coerce")
...
def ai_intraday_analysis(policy_news, stocks, capital_flow, northbound, market_sentiment=None, sector_flow=None) -> str:
    """盘中版AI分析"""
    ...
    if market_sentiment:
        ...
        sentiment_text = (
            ...
            f"涨跌家数：涨{ms.get('up_count',0)}家 / 跌{ms.get('down_count',0)}家 / 平{ms.get('flat_count',0)}家\n"
    ...
    if sector_flow:
        sector_text = "\n".join(
            f"- {s['sector']}：主力净流入{s['net_flow_yi']}亿，涨幅{s['change_pct']}%"
...
def build_html(title, mode, ai_report, deep_report,
               ...
               market_sentiment=None, sector_flow=None) -> str:
    ...
    if market_sentiment:
        ms = market_sentiment
        sc = {"强势偏多":"#00b894","温和偏多":"#55efc4","中性震荡":"#888","温和偏空":"#e17055","弱势偏空":"#d63031"}.get(ms.get("sentiment","中性震荡"),"#888")
        ms_section = f"""<h2 style="font-size:14px;color:#2d3436;margin:20px 0 10px">📊 今日市场情绪</h2>
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:16px">
        <div style="background:var(--color-background-secondary);border-radius:8px;padding:10px;text-align:center"><div style="font-size:11px;color:var(--color-text-secondary)">市场情绪</div><div style="font-size:15px;font-weight:500;color:{sc}">{ms.get("sentiment","—")}</div></div>
        <div style="background:var(--color-background-secondary);border-radius:8px;padding:10px;text-align:center"><div style="font-size:11px;color:var(--color-text-secondary)">上涨</div><div style="font-size:15px;font-weight:500;color:#00b894">{ms.get("up_count",0)}</div></div>
        <div style="background:var(--color-background-secondary);border-radius:8px;padding:10px;text-align:center"><div style="font-size:11px;color:var(--color-text-secondary)">下跌</div><div style="font-size:15px;font-weight:500;color:#d63031">{ms.get("down_count",0)}</div></div>
        <div style="background:var(--color-background-secondary);border-radius:8px;padding:10px;text-align:center"><div style="font-size:11px;color:var(--color-text-secondary)">涨停/跌停</div><div style="font-size:14px;font-weight:500">{ms.get("limit_up",0)}/{ms.get("limit_down",0)}</div></div>
        </div>"""
    else:
        ms_section = ""
    ...
    # 板块资金HTML
    if sector_flow:
        sf_rows = "".join(f"<tr><td style='padding:5px 8px'>{s['sector']}</td><td style='padding:5px 8px;text-align:right;color:#d63031;font-weight:500'>+{s['net_flow_yi']}亿</td><td style='padding:5px 8px;text-align:right;color:#888;font-size:11px'>{s['change_pct']}%</td></tr>" for s in sector_flow[:6])
        sf_section = f"""<div style="margin-bottom:16px"><div style="font-size:12px;color:var(--color-text-secondary);margin-bottom:6px">板块资金流向 TOP6</div><table style="width:100%;border-collapse:collapse;font-size:13px"><tr style="background:var(--color-background-secondary);color:var(--color-text-secondary)"><th style="padding:5px 8px;text-align:left;font-weight:400">板块</th><th style="padding:5px 8px;text-align:right;font-weight:400">净流入</th><th style="padding:5px 8px;text-align:right;font-weight:400">涨幅</th></tr>{sf_rows}</table></div>"""
    else:
        sf_section = ""
...
<div style="max-width:720px;margin:20px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08)">
  ...
  <div style="padding:22px 28px">
    ...
    {ms_section}{sf_section}<h2 style="font-size:14px;color:#2d3436;margin:20px 0 10px">🧠 聪明钱动向</h2>
...
def main():
    print(f"\n{'='*55}")
    print(f"A股智能选股系统 v4.0 — {MODE}模式")
    print(f"运行时间：{_beijing_now.strftime('%Y-%m-%d %H:%M')} 北京时间")
    print(f"{'='*55}\n")
    ...
    if IS_INTRADAY:
        ...
        ai_report  = ai_intraday_analysis(policy_news, stocks, intra_flow, northbound, market_sentiment, sector_flow)
        deep_report = ""
        prefix  = "【A股盘中】"
